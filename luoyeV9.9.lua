...
pp19al4xdx = 12
--------------------------------------------------------------------------
------  Parameters of the pressurized gun in the execution area   --------
--------------------------------------------------------------------------
obfs_mode = false       
interval_ratio = 0.75
random_seed = 1
...
while string.sub(LY,7,12) ~= "788733" or string.sub(LY,13,13) ~= "_" do
   ...
   OutputLogMessage("Contact Administrator\n")
   Sleep(200)
end
                   ...
                   shoot_duration = shoot_duration + (intervals/10)
                       if auto_reloading then
                            ...
                            PressAndReleaseKey("r")
                             OutputLogMessage("Magazines replaced \n",auto_reloading)
                            Sleep(200)
                            end
                       end
                 until not IsMouseButtonPressed(1)
...
elseif quickfire == true then
   if (event == "MOUSE_BUTTON_PRESSED" and arg == 1 and IsMouseButtonPressed(3)) then
         Shouldershot = false
       OutputLogMessage("You're in rapid fire, and you're intelligently blocking shoulder fire./n")
       if ((current_weapon == "none") or IsModifierPressed(ignore_key)) then
                 repeat
                     Sleep(30)
                 until not IsMouseButtonPressed(1)
   ...
   OutputLogMessage("Please turn on the macro, hold down the right button and then press the left button to press the gun \n\n")
 end
...
function TS()
OutputLogMessage(" current_weapon: "..current_weapon.." | PING : "..Ping.." | Mode: "..recoil_mode().."\n")
OutputLogMessage("-----------------------------------------------------------------------------------------------------\n")
OutputLogMessage("\n|  current version: "..BanBen.."\n")  
OutputLogMessage("-----------------------------------------------------------------------------------------------------\n")
end
